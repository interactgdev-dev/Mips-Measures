...
const Files = () => {
  ...
  const handleDownload = async (fileName) => {
    try {
      // Prefer direct browser download using Content-Disposition header
      const directUrl = `http://localhost:4700/files/${encodeURIComponent(
        fileName
      )}`;
      const a = document.createElement("a");
      a.href = directUrl;
      a.download = fileName; // optional; server also sets Content-Disposition
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    } catch (err) {
      console.error("Download failed (direct):", err);
      try {
        // Fallback to axios blob if direct fails
        const response = await axios({
          url: `http://localhost:4700/files/${encodeURIComponent(fileName)}`,
          method: "GET",
          responseType: "blob",
        });
        ...
        console.error("Download failed (blob):", e);
  ...
  const handleDelete = async (fileName) => {
    try {
      const response = await axios.delete(
        `http://localhost:4700/files/${fileName}`
      );
