...
/**
 * Bone density screening (M039) — matches utils/processing.js measure39.
 * Exclusions only on denominator rows; M039=1 once per patient so Counts = unique patients.
 */
const measure039 = async (collection, records) => {
  ...
  await bulkUpdateRecords(collection, records, (record) => {
    const patientKey = getPatientKey(record);
    const state = patientState.get(patientKey);
    const qualifies = state && state.hasDenominator && !state.excluded;
    let flag = 0;
    if (qualifies && !countedPatients.has(patientKey)) {
      flag = 1;
      countedPatients.add(patientKey);
    }
