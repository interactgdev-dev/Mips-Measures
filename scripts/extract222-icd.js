/**
 * One-off: pull icdCodesToCompare222 array literal from utils/processing.js into JSON.
 */
const fs = require("fs");
...
const key = "exports.measure222 = async";
const start = text.indexOf(key);
if (start === -1) throw new Error("measure222 block not found");
const rest = text.slice(start);
const icdMarker = "const icdCodesToCompare222 = [";
const icdPos = rest.indexOf(icdMarker);
if (icdPos === -1) throw new Error("icdCodesToCompare222 not found");
...
// Trusted repo file; array is only quoted strings.
const codes = Function(`"use strict"; return ${arrLiteral}`)();

const outPath = path.join(__dirname, "..", "utils", "measures", "data", "measure222LegacyIcd.json");
